...
int main(int argc, char *argv[])
{
    ...
    // Jongkyu, For Attach Each Process
    volatile int connect = 0;
    char hostname[256];
    gethostname(hostname, sizeof(hostname));
    printf("[Rank %d] PID %d on %s ready for attach\n", myid, getpid(), hostname);
    fflush(stdout);
    // In another terminal, gdb should set connect to non-zero after "gdb path; attach PID"
    while (connect == 0) sleep(5);
    ...
    else
    {
        ...
        if (dim == 3)
        {
            ...
            {
                Element *bel = mesh->GetBdrElement(b);
                const int attr = (b < NBE / 3) ? 3 : (b < 2 * NBE / 3) ? 1
                                                                       : 2;
                bel->SetAttribute(attr);
            }
        }
    ...
    if (product == num_tasks || cartesian_partitioning)
    {
        ...
        int *partitioning = cartesian_partitioning ? mesh->CartesianPartitioning(cxyz) : mesh->CartesianPartitioning(nxyz);
    ...
    int source = 0;
    bool visc = true, vorticity = false;
    switch (problem)
    {
    case 0:
...
static void Checks(const int dim, const int ti, const double nrm, int &chk)
{
    ...
    if (dim == 2)
    {
        ...
        {
            chk++;
            MFEM_VERIFY(rerr(nrm, p0_05, eps), "P0, #05");
        }
        if (pb == 0 && ti == 27)
        {
            chk++;
            MFEM_VERIFY(rerr(nrm, p0_27, eps), "P0, #27");
        }
        ...
        {
            chk++;
            MFEM_VERIFY(rerr(nrm, p1_05, eps), "P1, #05");
        }
        if (pb == 1 && ti == 15)
        {
            chk++;
            MFEM_VERIFY(rerr(nrm, p1_15, eps), "P1, #15");
        }
        ...
        {
            chk++;
            MFEM_VERIFY(rerr(nrm, p2_05, eps), "P2, #05");
        }
        if (pb == 2 && ti == 59)
        {
            chk++;
            MFEM_VERIFY(rerr(nrm, p2_59, eps), "P2, #59");
        }
        ...
        {
            chk++;
            MFEM_VERIFY(rerr(nrm, p3_05, eps), "P3, #05");
        }
        if (pb == 3 && ti == 16)
        {
            chk++;
            MFEM_VERIFY(rerr(nrm, p3_16, eps), "P3, #16");
        }
        ...
        {
            chk++;
            MFEM_VERIFY(rerr(nrm, p4_05, eps), "P4, #05");
        }
        if (pb == 4 && ti == 18)
        {
            chk++;
            MFEM_VERIFY(rerr(nrm, p4_18, eps), "P4, #18");
        }
        ...
        {
            chk++;
            MFEM_VERIFY(rerr(nrm, p5_05, eps), "P5, #05");
        }
        if (pb == 5 && ti == 36)
        {
            chk++;
            MFEM_VERIFY(rerr(nrm, p5_36, eps), "P5, #36");
        }
        ...
        {
            chk++;
            MFEM_VERIFY(rerr(nrm, p6_05, eps), "P6, #05");
        }
        if (pb == 6 && ti == 36)
        {
            chk++;
            MFEM_VERIFY(rerr(nrm, p6_36, eps), "P6, #36");
        }
        ...
        {
            chk++;
            MFEM_VERIFY(rerr(nrm, p7_05, eps), "P7, #05");
        }
        if (pb == 7 && ti == 25)
        {
            chk++;
            MFEM_VERIFY(rerr(nrm, p7_25, eps), "P7, #25");
        }
    }
    if (dim == 3)
    {
        ...
        {
            chk++;
            MFEM_VERIFY(rerr(nrm, p0_188, eps), "P0, #188");
        }
        ...
        {
            chk++;
            MFEM_VERIFY(rerr(nrm, p1_28, eps), "P1, #28");
        }
        ...
        {
            chk++;
            MFEM_VERIFY(rerr(nrm, p7_24, eps), "P7, #24");
        }
    }
}